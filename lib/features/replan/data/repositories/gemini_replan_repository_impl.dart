...
class GeminiReplanRepositoryImpl implements ReplanRepository {
  ...
  GeminiReplanRepositoryImpl({FirebaseFirestore? firestore, FirebaseAuth? auth})
...
''';
      ...
      List<ProposedSwap> swaps = _generateSwaps(
        oldItinerary.days,
        newItinerary.days,
      );

      return ReplanResult(swaps: swaps, newItinerary: newItinerary);
    } catch (e) {
  ...
  List<ProposedSwap> _generateSwaps(
    List<ItineraryDay> oldDays,
    List<ItineraryDay> newDays,
  ) {
    ...
    final int minLength = oldActivities.length < newActivities.length
  ...
  Future<void> acceptReplan({
    ...
    try {
      ...
      await tripDocRef.update({'itinerary': newItinerary.toMap()});
    } catch (e) {
